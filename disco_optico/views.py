...
class ImageViewSet(viewsets.ModelViewSet, mixins.CreateModelMixin):
    ...
    def create(self,request):
        ...
        blur = cv2.GaussianBlur(crop_image,(13,13),0)
        z = blur.reshape(-1,1)
        z = np.float32(z)
        ...
        # Optic cup circle
        circles = cv2.HoughCircles(maxima,cv2.HOUGH_GRADIENT,1,250,param1=7,param2=10,minRadius=0,maxRadius=int(maxima.shape[1]*0.5))
        ...
        # Optic disc circle
        circles = cv2.HoughCircles(second,cv2.HOUGH_GRADIENT,1,250, param1=30,param2=30,minRadius=0,maxRadius=0)
        circles = np.uint16(np.around(circles))
        for j in circles[0,:]:
        ...
        ## Glaucoma diagnosis
        CDR =  pixels_optic_cup / pixels_optic_disc
        if CDR > 0.5:
            print("Presenta glaucoma, ",CDR)
        else:
            print("Es una persona sana, ",CDR)
